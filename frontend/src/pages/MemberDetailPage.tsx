        ...
        <div className="mx-auto flex max-w-3xl flex-col gap-4">
          {!loaded ? (
            ...
            <>
              ...
              {tab === 'payments' ? <PaymentsTab load={loadPayments} /> : null}
            </>
          )}
        </div>
      </NavShell>
...
function PaymentsTab({ load }: { load: () => Promise<MemberPaymentsStubDto> }) {
  const [data, setData] = useState<MemberPaymentsStubDto | null>(null)
  ...
  // gym-management-member-profile-extension.md §5: explicit "not available"
  // state, deliberately distinct from "loaded, zero payments."
  return (
    <Card>
      <div className="py-6 text-center">
        <p className="text-sm font-medium text-ink">Billing not yet enabled</p>
